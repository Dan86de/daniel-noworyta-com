			...
			>
				<Headless.ListboxSelectedOption
					...
					className={clsx([
						...
						// Typography
						"text-left text-base/6 text-neutral-950 placeholder:text-neutral-500 sm:text-sm/6 dark:text-white forced-colors:text-[CanvasText]",
						// Border
						...
						// Disabled state
						"group-data-disabled:border-neutral-950/20 group-data-disabled:opacity-100 dark:group-data-disabled:border-white/15 dark:group-data-disabled:bg-white/[2.5%] dark:group-data-disabled:data-hover:border-white/15",
					])}
				/>
				<span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
					<svg
						className="size-5 stroke-neutral-500 group-data-disabled:stroke-neutral-600 sm:size-4 dark:stroke-neutral-400 forced-colors:stroke-[CanvasText]"
						viewBox="0 0 16 16"
						aria-hidden="true"
						fill="none"
					>
...
>) {
	const sharedClasses = clsx(
		...
		"*:data-[slot=icon]:text-neutral-500 group-data-focus/option:*:data-[slot=icon]:text-white dark:*:data-[slot=icon]:text-neutral-400",
...
export function ListboxDescription({
	...
	return (
		<span
			...
			className={clsx(
				className,
				"flex flex-1 overflow-hidden text-neutral-500 before:w-2 before:min-w-0 before:shrink group-data-focus/option:text-white dark:text-neutral-400",
			)}
		>

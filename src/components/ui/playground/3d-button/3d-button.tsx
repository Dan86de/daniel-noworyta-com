...
import baseAsset from "./assets/base.svg";
import buttonAsset from "./assets/button.svg";
import coverAsset from "./assets/cover.svg";

interface D3ButtonProps {
	label: string;
	className?: string;
	onClick?: () => void;
}

export const D3Button = (props: D3ButtonProps) => {
	return (
		<button
			type="button"
			className={cn("frame select-none", props.className)}
			onClick={props.onClick}
		>
			<img className="base" src={baseAsset} alt="base-image" />
			<img className="button" src={buttonAsset} alt="button-image" />

			<svg
				className="recolor-overlay"
				width="400"
				height="400"
				viewBox="0 0 400 400"
				fill="none"
			...
			<img className="button-cover " src={coverAsset} alt="cover-image" />
